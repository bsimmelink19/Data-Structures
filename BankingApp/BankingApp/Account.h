...
protected:
	...
	std::string get_fees()
	{
		...
		//sets the overdraft penalty to the correct penalty, depending on customer type 
		overdraft = customer->get_OVERDRAFT_PENALTY();
		//sets charge to the correct check charge, depending on customer type
...
protected:
	...
	*/
	void add_interest(double interest) {
		...
		Transaction *tran = new Transaction(customer->get_customer_number(), type, amt, get_fees());
...
public:
	...
	*/
	virtual std::string to_string() {
		std::stringstream ss; // for composing the string that describes this account
						  ...
						  //displays customer's name and customer number
	...
	*/
	virtual void withdraw(double amt) {
		//checks if the withdrawal will exceed the balance 
		if (balance - amt > 0)
		{
		...
		//displays an error if the requested amount exceeds the balance
