	...
	void update(int clock)
	{
		...
		if (the_queue.empty()) {
			...
			if (!landing_queue->the_queue.empty()) {
				...
				int wait_in_landing = clock - plane->arrival_time;

				// FIXME: update total_wait and num_served for the landing queue
				landing_queue->total_wait = landing_queue->total_wait + wait_in_landing;
				...
				*/
				int range = max_service_time - min_service_time;
				plane->service_time = my_random.next_int(range);
