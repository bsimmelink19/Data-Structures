...
bool PlaceQueen(int column) {
	...
	if (column == NUMBER_COLS) { // Base Condition
		//If the solutions vector is empty, the new solution can't have been found yet
		//The new solution is stored in the solutions vector, PlaceQueen is called again, starting from column 0
		if (PlacementSolutions.empty()) 
		{
			PlacementSolutions.push_back(RowPlacement);
			return PlaceQueen(0);
		}
		//compares all solutions in the solutions vector to the new solution, 
		//if that solution does not match any already found it is added to the solution vector
		for (int i = 0; i < PlacementSolutions.size(); ++i)
		{
			for (int j = 0; j < RowPlacement.size(); ++j)
			{
				if (PlacementSolutions[i][j] != RowPlacement[j])
				{
					PlacementSolutions.push_back(RowPlacement);
					return PlaceQueen(0);
				}
